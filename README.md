...
| `n_bulk_metrics` | Sugar over `n_bulk` for the Metrics DB: log many same-date metrics at once (`{date, entries:[{metric, value, unit?, memo?}], mode?}`). Maps to Notion properties 指標/値/単位/メモ. `mode: "upsert"` (default, idempotent) / `"skip_existing"` / `"create"` — `create` forces a duplicate row, use it only when you want one. `metric` names normalized via `METRIC_ALIASES` env var |
