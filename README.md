...
| `n_metrics_series` | Trend-read sugar over the Metrics DB: one call returns `{series:[{date,値,…}], stats:{first,last,delta,min,max,avg,median,count}}` for a single `metric` with optional `from`/`to` date bounds. When `METRIC_RANGES` env var defines a reference range for the metric, also returns `ref:{low,high}` + `last_flag:"high"/"low"/"normal"`. Replaces the n_query→filter→sorts→stats boilerplate |
